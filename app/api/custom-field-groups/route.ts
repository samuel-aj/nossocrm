...
// GET devolve na ordem manual (position, migração 20260902130000) e, sem ela,
// em ordem alfabética; PATCH { names } grava a ordem manual.
...
const ReorderSchema = z.object({
  names: z.array(z.string().min(1).max(60)).min(1).max(200),
}).strict();
...
export async function GET() {
  ...
  // Com a coluna position: ordem manual, depois alfabética. Sem ela (migração
  // pendente), cai na consulta antiga em ordem alfabética.
  ...
  if (!ordered.error) return NextResponse.json({ data: (ordered.data || []).map(r => r.name as string) });
...
/** Ordem manual dos grupos: position = índice na lista enviada. */

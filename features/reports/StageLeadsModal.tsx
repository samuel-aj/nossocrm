      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead className="sticky top-0 bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400">
            <tr>
              {['Nome do lead', 'Cria??o', 'Qualifica??o', 'Encerramento'].map(label => (
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {leads.map(lead => (
              <tr key={lead.id} className="text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                <td className="px-5 py-3 font-medium min-w-48 text-slate-900 dark:text-white">{lead.title}</td>
            ...
            {!leads.length && <tr><td colSpan={4} className="px-5 py-10 text-center text-slate-500">Nenhum lead nesta etapa no per?odo selecionado.</td></tr>}
          </tbody>
        </table>
      </div>
    </Modal>

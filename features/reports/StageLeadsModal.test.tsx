...
it('mostra as quatro colunas, datas reais, vazios e permite fechar', () => {
  ...
  expect(within(dialog).getAllByRole('columnheader').map(cell => cell.textContent)).toEqual(['Nome do lead', 'Cria??o', 'Qualifica??o', 'Encerramento']);
  expect(within(screen.getByText('Ana').closest('tr')!).getAllByRole('cell').map(cell => cell.textContent)).toEqual(['Ana', '02/08/2026', '10/08/2026', '20/08/2026']);
  expect(within(screen.getByText('Bruno').closest('tr')!).getAllByRole('cell').map(cell => cell.textContent)).toEqual(['Bruno', '03/08/2026', '', '']);

...
export function ReportFiltersModal({ filters, owners, products, onApply, onClose }: ReportFiltersModalProps) {
  const [draft, setDraft] = useState(filters);

  return (
    <Modal isOpen onClose={onClose} title="Filtros do relatório" size="md" className="self-center">
      <form className="flex flex-col gap-4" onSubmit={event => { event.preventDefault(); onApply(draft); }}>
        ...
        <div className="flex justify-end gap-3 border-t border-slate-200 pt-5 pb-[env(safe-area-inset-bottom)] dark:border-white/10">

  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`flex items-center gap-2 whitespace-nowrap px-4 py-2 max-md:shrink-0 max-md:px-3 max-md:py-1.5 max-md:text-xs rounded-lg text-sm font-medium transition-all border ${
          activeStage === 'ALL'
...
export const StageBadge: React.FC<{ stage: ContactStage | string }> = ({ stage }) => {
  ...
  if (!config) {
    return (
      <span className="inline-flex items-center gap-1 whitespace-nowrap text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
        {stage}
      </span>
    );
  }
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 whitespace-nowrap text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${config.activeColor}`}
    >
      {config.label}
    </span>
  );

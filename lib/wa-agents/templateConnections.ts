...
export async function botTemplateConnectionError(admin: SupabaseClient, organizationId: string, steps: BotStep[], connectionIds: string[]): Promise<string | null> {
  ...
  const { data, error } = await admin.from('message_templates')
    .select('id, name, type, connection_id, meta_status, buttons, header_type, media_id')
  ...
  for (const id of ids) {
    ...
    if (template.header_type && !template.media_id) return `O modelo "${template.name}" precisa de mídia em Configurações → Modelos.`;
